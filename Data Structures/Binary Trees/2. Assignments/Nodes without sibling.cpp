...
void printNodesWithoutSibling(BinaryTreeNode<int> *root) {
    ...
    //Checking if there is any node without any sibling
    
    //Here if left node is NULL that means right node has no sibling
    if(root->left==NULL && root->right!=NULL) {
        cout<<root->right->data<<" ";
    }
    
    //Here if right node is NULL that means left node has no sibling 
    else if(root->right==NULL && root->left!=NULL) {
        cout<<root->left->data<<" ";
    }
    
    //Recursively calling for both left and right nodes
